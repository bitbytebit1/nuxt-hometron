    ...
    <v-row>
      <v-col v-for="(item, i) in items" :key="i" cols="12">
        <v-card>
          ...
          <v-card-actions>
            <v-btn @click="action('WinActivate', [item.handle])">
              <v-icon>mdi-flip-to-front</v-icon>
              Activate
            </v-btn>
            <v-btn @click="action('WinSetState', [item.title, '','@SW_MINIMIZE'])">
              <v-icon>mdi-window-minimize</v-icon>
              Minimize
            </v-btn>
            <v-btn @click="action('WinSetState', [item.title, '','@SW_MAXIMIZE'])">
              <v-icon>mdi-window-maximize</v-icon>
              Maximise
            </v-btn>
            <v-btn @click="action('WinKill', [item.title])">
